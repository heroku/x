...
// nolint: lll
func MustProvider(ctx context.Context, logger zerolog.Logger, cfg Config, service, serviceNamespace, stage, serviceInstanceID string, opts ...otel.Option) metrics.Provider {
	// This provider is used for metrics reporting to the  collector.
	logger.Info().Str("metrics_destinations", strings.Join(cfg.MetricsDestinations, ",")).Msg("setting up  provider")
	...
	if endpoint == nil {
		logger.Fatal().Msg("provider collectorURL cannot be nil")
	}
	...
	if err != nil {
		logger.Fatal().Err(err).Msg("failed to create otel provider")
	}

	if err := otelProvider.(*otel.Provider).Start(); err != nil {
		logger.Fatal().Err(err).Msg("failed to start otel provider")
	}
