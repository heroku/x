...
func NewLogger(cfg Config) zerolog.Logger {
	...
	logctx := logger.With().Str("app", cfg.AppName).
		Str("deploy", cfg.Deploy)

	if cfg.SpaceID != "" {
		logctx = logctx.Str("space", cfg.SpaceID)
	}
	if cfg.Dyno != "" {
		logctx = logctx.Str("dyno", cfg.Dyno)
	}

	return logctx.Logger()
}

// ReportPanic attempts to report the panic to rollbar via zerolog.
func ReportPanic(logger zerolog.Logger) {
	if p := recover(); p != nil {
		logger.Panic().Msg(p.(string))
	}
}
