...
// Setup installs a Rollbar report handler to the default zerolog logger.
//
// Setup is skipped if Token and Env are not present in the config.
func Setup(logger zerolog.Logger, cfg Config) zerolog.Logger {
	if cfg.Token == "" && cfg.Env == "" {
		logger.Info().Str("at", "skipping-rollbar").Msg("")
		return logger
	}
	...
	hook := rollrus.NewHook(cfg.Token, cfg.Env,
		...
		rollrus.WithLevels(zerolog.ErrorLevel, zerolog.PanicLevel, zerolog.FatalLevel),

	TODO: Figure out if timestamp disabling is needed.
