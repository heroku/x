...
// error to clients.
func LoggingUnaryPanicHandler(logger zerolog.Logger) grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (resp interface{}, err error) {
		defer handleCrash(func(r interface{}) {
			werr := errors.Errorf("grpc unary server panic: %v", r)
			logger.Error().Err(werr).Msg("grpc unary server panic")
...
func LoggingStreamPanicHandler(logger zerolog.Logger) grpc.StreamServerInterceptor {
	return func(srv interface{}, stream grpc.ServerStream, info *grpc.StreamServerInfo, handler grpc.StreamHandler) (err error) {
		defer handleCrash(func(r interface{}) {
			werr := errors.Errorf("grpc stream server panic: %v", r)
			logger.Error().Err(werr).Msg("grpc stream server panic")
			err = toPanicError(werr)
		})
