...
func RunStandardServer(logger zerolog.Logger, port int, serverCACerts [][]byte, serverCert, serverKey []byte, server Starter, opts ...ServerOption) error {
...
// (if provider passed), panic handling, a health check service, TLS termination
// with client authentication, and proxy-protocol wrapping.
func NewStandardServer(logger zerolog.Logger, port int, serverCACerts [][]byte, serverCert tls.Certificate, server Starter, opts ...ServerOption) cmdutil.Server {
	tls, err := TLS(serverCACerts, serverCert)
	if err != nil {
		logger.Fatal().Err(err).Send()
	}

	opts = append(opts, tls, LogContext(logger.With().Str("component", "grpc")))
	grpcsrv := New(opts...)
