...
func RunStandardServer(logger *slog.Logger, port int, serverCACerts [][]byte, serverCert, serverKey []byte, server Starter, opts ...ServerOption) error {
...
// (if provider passed), panic handling, a health check service, TLS termination
// with client authentication, and proxy-protocol wrapping.
func NewStandardServer(logger *slog.Logger, port int, serverCACerts [][]byte, serverCert tls.Certificate, server Starter, opts ...ServerOption) cmdutil.Server {
	...
	opts = append(opts, tls, Logger(logger.With(slog.String("component", "grpc"))))
	grpcsrv := New(opts...)
